...
import { TextField as DefaultTextField, FormHelperText } from '@mui/material';
...
const parseTime = (value: any) => {
    let new_hours = value
    if (value && value?.format() === VALIDATION.INVALID_DATE) {
...
export const TimePickerInput = ({
    ...
    const { id, input, isRequired, meta } = useInput({
        ...
        const setValueChange = (newValue:any) =>{
            ...
            if(newValue && newValue?.format() === 'Invalid date'){
                ...
                if (parseInt(val[1]) > NUMBER.FIFTYNINE){
                    setError('Minutes must be less than 60')
                }else{
                    setError('')
                }
                if(val.length === NUMBER.TWO){
    ...
    return (
        <LocalizationProvider dateAdapter={DateAdapter}>
            <TimePicker
                id={id}
                {...input}
                onChange={(newValue) => setValueChange(newValue)}
                variant={variant}
                margin={margin}
                ...
                renderInput={(params) => 
                        {
                    return  <DefaultTextField {...params}
                            error={!!(touched && (error || submitError))}
                        label={
                            <FieldTitle
                                label={label}
