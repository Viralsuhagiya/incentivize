...
export default (introspectionResults: IntrospectionResult) => (
    ...
    console.log(`${raFetchMethod} ${resource.type.name} ${queryType.name} Preparing params => `, JSON.stringify(params));
    const preparedParams = prepareParams(
        params,
        queryType,
        introspectionResults
    );
    console.log(`${raFetchMethod} ${resource.type.name} ${queryType.name} Preparing params => `, JSON.stringify(preparedParams));
    ...
    switch (raFetchMethod) {
        ...
        case GET_MANY:
            const ids = preparedParams.ids.map((val:number|{'id':number})=>{
        ...
        case GET_MANY_REFERENCE: {
            ...
            variables.filter = {
                ...variables.filter,
                [preparedParams.target]: {_eq:preparedParams.id || 0},
            };
...
const buildGetListVariables = (introspectionResults: IntrospectionResult) => (
    ...
    if (params.filter) {
        variables.filter = Object.keys(params.filter).reduce((acc, key) => {
            if (key === 'ids') {
                return { ...acc, id: {_in: params.filter[key]} };
            }
            ...
            if (typeof params.filter[key] === 'object') {
                ...
                const filterField = (type as IntrospectionInputObjectType)?.inputFields?.find(
                    t => t.name === `${key}`
                );
                ...
                if (filterField) {
                    return {
                        ...acc,
                        [key]: sanitizeValue(getFinalType(filterField.type), params.filter[key]),
                    };
                }
    ...
    if(params.groupBy){
        variables.groupBy = params.groupBy;
    };
    if(params.fields){
        variables.fields = params.fields;
    };
    variables.lazy = params.lazy;
    return variables;
};
...
const buildCreateUpdateVariables = (
    ...
    if(raFetchMethod===CREATE) {
        const changeset = transform_obj_keys(params.data);
        params = {data: {input: changeset}};
    }
    else if(raFetchMethod===UPDATE) {
        const changeset = prepareChangeset(params.previousData, params.data);
        changeset.id = parseInt(params.id)
        params = {data: {input: changeset}}
    };
