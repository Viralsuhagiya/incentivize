...
describe('testbuildFields', () => {
    it('returns an object with the fields to retrieve', () => {
        const PayrollResourceType = {
        }
        const OtInput = {
            name: 'OtInput',
            fields: [
                { name: 'id', type: { kind: TypeKind.SCALAR, name:'Int', }},
                { name: 'hours', type: { kind: TypeKind.SCALAR, name:'Float', }},
                { name: 'create_date', type: { kind: TypeKind.SCALAR, name:'DateTime', }},
            ],
        }
        const introspectionResults = {
            resources: [
                    { type: { name: 'Payroll', kind: TypeKind.OBJECT }, expandFields:['overtime_input_ids']},
                    { type:OtInput }
                    ],
            types: [
                OtInput
        ...
        const fields = [
            { name: 'id', type: { kind: TypeKind.SCALAR, name:"Int", }},
            { name: 'overtime_input_ids', type: {kind: TypeKind.LIST, name: null, ofType: { kind: TypeKind.OBJECT, name: 'OtInput'}}},
        ];

        expect(print(buildFields(introspectionResults)(fields,introspectionResults.resources[0].expandFields))).toEqual([
            'id',
            `overtime_input_ids {
  id
  hours
        ...
        expect(
            print(
                buildApolloArgs(
                    ...
                    { foo: 'foo_value', barId: 100, barIds: [NUMBER.HUNDRED_ONE, NUMBER.HUNDRED_TWO] }
                )
            )
