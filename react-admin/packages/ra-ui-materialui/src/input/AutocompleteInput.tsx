...
import { ResettableTextFieldClasses } from './ResettableTextField';
import EditIcon from '@mui/icons-material/Edit';
import { useSupportEditSuggestion } from './useSupportEditSuggestion';
...
If you provided a React element for the optionText prop, you must also provide the matchSuggestion prop (used to match the user input with a choice)`);
    ...
    const handleEdit = () => {
        handleChangeWithEditSupport(selectedItem)
        setopenOnFocus(false)
    };
    const {
        handleChange: handleChangeWithEditSupport,
        editElement,
    } = useSupportEditSuggestion({
        edit,
        handleChange,
    });
    const {
    ...
    const handleInputChange = (event: any, newInputValue: string) => {
        if (newInputValue == createLabel || newInputValue == translate('ra.action.create')){
            return
        }
        ...
        <>
            <Autocomplete
                ...
                renderInput={params => {
                    ...
                    const endEditAdornment = (<InputAdornment position="end"
                        classes={{
                    ...
                    const endAdornment = <>{params.InputProps.endAdornment}{endEditAdornment}</>
                    const InputProps = { ...params.InputProps, endAdornment: selectedItem && edit? endAdornment : params.InputProps.endAdornment }
                    const textParams = { ...params,InputProps:InputProps}
                    return(
                        <TextField
                            {...textParams}
                            id={id}
                ...
                renderOption={(props, record) => {
                    if ((record as Record).id == "@@ra-create"){
                        return <li style={{color: theme.palette.primary.main}} {...props}>{getChoiceText(record)}</li>;
                    }
                    if (isValidElement(optionText)) {
