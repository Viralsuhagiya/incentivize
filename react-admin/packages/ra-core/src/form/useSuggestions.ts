...
export const getSuggestionsFactory = ({
    ...
    if (allowEmpty) {
        //there is some bug that its not removing duplicates
        //and because of that two empty items are coming in dropdown
        //so for that fix do check duplicates here before adding for empty items.
        const emptySuggestion = getSuggestion({
            optionText,
            optionValue,
            text: emptyText,
            value: emptyValue,
        })
        if(findIndex(suggestions, emptySuggestion)===-1){
            suggestions.unshift(emptySuggestion);
        }
    }
