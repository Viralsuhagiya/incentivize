...
export const buildQueryFactory = (
    ...
    const buildQuery: BuildQuery = (raFetchType, resourceName, params) => {
        ...
        console.log(`SUPER ${raFetchType} ${resource.type.name} ${queryType.name} Variables => `, JSON.stringify(variables));
        console.log(`SUPER ${raFetchType} ${resource.type.name} ${queryType.name} Query => `, query);
